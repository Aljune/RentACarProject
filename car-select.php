                ...
                <form method="post" action="reservation-confirm.php">
                    <div class="card-body">
                           ...
                           <div class="row rows-col-2 mb-3"> 
                            ...
                            <div class="row rows-col-3 mb-3"> 
                                <div class="form-floating col-md-4 ">
                                    <input type="text" name="drop_location" class="form-control" id="floatingInput" placeholder="DROP LOCATION" >
                                    <label style="left: 12px;" for="floatingInput">DROP LOCATION</label>
                                </div>
                                <div class="form-floating col-md-4 ">
                                    <input type="date" name="drop_date" class="form-control" id="floatingPassword" placeholder="DROP DATE" >
                                    <label style="left: 12px;" for="floatingPassword">DROP DATE</label>
                                </div>
                                <div class="form-floating col-md-4 ">
                                    <input type="time" name="drop_time" class="form-control" id="floatingPassword" placeholder="DROP TIME" >
            ...
            <div class="card my-3">
                ...
                <div class="card-body">
                    <div class="text-center d-flex justify-content-center align-items-center">
                        <h4>Total: </h4> <h2> P<?php echo $total_rate; ?></h2>
                    </div>  
                    <hr/>
                    <div class="d-flex  ">
                    ...
                    <div class="d-flex justify-content-between">
                        <h5><?php echo $selectedInfoCar['type']?></h5>
                        <h5><?php echo $selectedInfoCar['status'] =='AVAILABLE' ? 'Available' : 'Not Available' ?></h5>
                    </div>
                </div>
            </div>
        </div>
...
<script>
   ...
   // get the radio buttons by their name
    var radioButtons = document.getElementsByName("payment_type");

    // add a change event listener to each radio button
    for (var i = 0; i < radioButtons.length; i++) {
    radioButtons[i].addEventListener("change", function() {
        // get the value of the selected radio button by its ID
        var selectedPaymentType = document.querySelector('input[name="payment_type"]:checked').value;
        if(selectedPaymentType === 'gcash'){
            $('.gcash-form').addClass('d-flex');
            $('.gcash-form').removeClass('d-none');

            $('.credit-form').removeClass('d-flex');
            $('.credit-form').addClass('d-none');
        }
        if(selectedPaymentType === 'credit'){
            $('.credit-form').addClass('d-flex');
            $('.credit-form').removeClass('d-none');

            $('.gcash-form').removeClass('d-flex');
            $('.gcash-form').addClass('d-none');
        }
        //  alert("Selected gender is " + selectedPaymentType);
    });
    }
